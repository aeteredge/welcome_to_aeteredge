...
This document outlines the structure and required fields for the `payment-invoices` payload using the **`payment_card_clp_hpp`** service, which supports **card-based pay-ins in Chile (CLP)** through a **Hosted Payment Page (HPP)**.
...
| `service`             | Must be `"payment_card_clp_hpp"`.                   |
...
- The `payment_card_clp_hpp` service allows customers to complete payments using **debit or credit cards**.
- The card data collection and authentication are handled via a **Hosted Payment Page (HPP)**.
