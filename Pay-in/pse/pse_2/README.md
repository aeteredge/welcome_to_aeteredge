...
This document outlines the structure and required fields for the `payment-invoices` payload using the **`pse_cop_hpp`** service, which supports **PSE bank transfer pay-ins in Colombia (COP)** through a **Hosted Payment Page (HPP)**.
...
| `individual_tax_id` | Customer identification number (e.g. Cédula / NIT).  |
...
- The `pse_cop_hpp` service redirects the customer to their bank via **PSE** to complete the payment.
