...
This document outlines the structure and required fields for the `payment-invoices` payload using the **`bank_transfer_pen_hpp`** service, which supports **bank transfer pay-ins in Peru (PEN)** through a **Hosted Payment Page (HPP)**.
...
| `service`             | Must be `"bank_transfer_pen_hpp"`.                  |
...
- The `bank_transfer_pen_hpp` service uses a **Hosted Payment Page (HPP)** where the customer is redirected to complete the bank transfer.
