...
This document outlines the structure and required fields for the `payment-invoices` payload using the **`efecty_cop_hpp`** service, which supports **Efecty cash-based pay-ins in Colombia (COP)** through a **Hosted Payment Page (HPP)**.
...
| `service`             | Must be `"efecty_cop_hpp"`.                         |
...
- The `efecty_cop_hpp` service generates a **payment reference** that the customer uses to complete the payment at an Efecty location.
